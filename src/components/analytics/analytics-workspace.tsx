...
function GeneratorAnalytics({ generators }: { generators: GeneratorAnalyticsItem[] }) {
  ...
  const comparisonData = useMemo(() => {
    ...
    return generators.flatMap((generator) => {
      ...
      return value === null ? [] : [{ generator: generator.generatorId, value, valueLabel: displayValue(value, selectedMetric.unit) }];
    });
  ...
  return (
    <section className="border border-slate-200 bg-white">
      <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h3 className="text-base font-semibold text-slate-950">Generator Analytics</h3>
          <p className="mt-1 text-sm text-slate-500">{generators.length} registered generator{generators.length === 1 ? "" : "s"}</p>
        </div>
        <div className="inline-flex w-fit rounded-md border border-slate-300 bg-slate-50 p-1" role="tablist" aria-label="Generator analytics view">
          <button
            type="button"
            role="tab"
            aria-selected={view === "profile"}
            onClick={() => setView("profile")}
            className={`inline-flex min-h-9 items-center gap-2 rounded px-3 text-sm font-semibold ${view === "profile" ? "bg-white text-slate-950 shadow-sm" : "text-slate-600"}`}
          >
          ...
          <button
            ...
            className={`inline-flex min-h-9 items-center gap-2 rounded px-3 text-sm font-semibold ${view === "compare" ? "bg-white text-slate-950 shadow-sm" : "text-slate-600"}`}
          >
      ...
      {view === "profile" && selectedGenerator ? (
        <div className="p-5">
          <div className="grid gap-4 border-b border-slate-200 pb-5 lg:grid-cols-[minmax(260px,1fr)_2fr] lg:items-end">
