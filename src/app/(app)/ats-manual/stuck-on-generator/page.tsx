...
import { atsStuckOnGeneratorItems } from "@/lib/module-definitions";

export default function AtsStuckOnGeneratorPage() {
  return (
    <div className="space-y-5">
      <PageHeader title="ATS Stuck on Generator" description="Emergency manual operation procedure for returning the ATS from generator side to stable mains." />
      <section className="rounded-md border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-5 py-4">
          <h3 className="text-base font-semibold text-slate-950">Procedure Steps</h3>
        </div>
        <ol className="divide-y divide-slate-100">
          {atsStuckOnGeneratorItems.map((item, index) => (
